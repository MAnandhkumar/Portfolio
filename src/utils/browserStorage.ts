...
class BrowserStorage implements BrowserStorageType {
  ...
  setItem<T>(key: string, value: T): void {
    ...
    try {
      ...
      console.error(`Error setting storage value for key "${this.keyPrefix + key}":`, error);
    }
  }
  ...
  removeItem(key: string): void {
    ...
    try {
      ...
      console.error(`Error removing storage value for key "${this.keyPrefix + key}":`, error);
    }
  }
...
const createStorageAccessor = (instance: BrowserStorage) => {
  return <T>(key: string, value?: T): [() => T | null, (value: T) => void, () => void] => {
    if (!key) throw new Error("Invalid key: Key must not be empty");
    ...
    return [
      ...
      (value: T) => instance.setItem(key, value), // Setter
 ...
 */
const ENABLE_ENCRYPTION: boolean = CFG.storage_encryption;
const KEY_PREFIX: string = (process.env.NEXT_PUBLIC_STORAGE_PREFIX as string) || "APP_";
...
const localStorageInstance = new BrowserStorage({
  storageObject: isBrowser ? window.localStorage : dummyStorage,
...
const sessionStorageInstance = new BrowserStorage({
  storageObject: isBrowser ? window.sessionStorage : dummyStorage,
  keyPrefix: KEY_PREFIX,
